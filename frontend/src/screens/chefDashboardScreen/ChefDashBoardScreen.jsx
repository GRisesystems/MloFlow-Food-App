...
const TabButton = styled.button` // <-- Add backtick here
  flex: 1;
  padding: 20px;
...
const ChefDashboard = () => {
  ...
  const [acceptedRequests, setAcceptedRequests] = useState([]); // Maintain state here
  const [deniedRequests, setDeniedRequests] = useState([]); // Maintain state here
  const [selectedRequest, setSelectedRequest] = useState(null); // Specify the type as 'number | null'
